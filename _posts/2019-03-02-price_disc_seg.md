...
subtitle: clever trick in tech (with examples)
published: true
enable_latex: true
...
**Why is their such a large price discrepancy between the Titan XP & 1080TI?**  The features of the Titan XP (it's higher memory, more cuda cores, higher bandwidth) makes it ideal for advanced computing. A typical video game does not need these specs to play a game "better", but data centers and ML platforms do. As a result, Nvidia can use these features to let their serious consumers separate from the hobbyists - and charge them more for it.

People will point out that developing chips is a complicated process that doesn't yield perfect chips consistently. Batches of GP102 could have come in "perfect" and "okay" qualities. The "perfect" chips meet the spec and can be put into TitanXP's while the "okay" batches are partially defective. Instead of just throwing away the chips, companies (i.e Nvidia, Intel, AMD) will throw them into a lower-tier product and disable key features on them (1080TI). My counterpoint is that there are also times when manufacturers do have good yields, and disables key features to put the "perfect" chips into lesser products anyways.
...
A different concern regarding "Big Data" is that companies can uncover medical conditions that are prohibited from being factored or discovering health issues that the consumer is unaware of. [A common anecdote is how Target's marketing team sent pregnancy advertisements to a girl before she knew she was pregnant](https://www.nytimes.com/2012/02/19/magazine/shopping-habits.html). I personally dislike how hyped this Target anecdote. The way people frame it implies that Target has some omniscient AI, but we don't know the accuracy of this system at all. It might have sent ads to a million non-pregnant people, and it happened to be right once! The concern is still there as using unclear, unverifiable classification systems shouldn't be used to determine such important things like insurance (and what Cathey O'Neil argues as well). Read chapter 9 of "Weapons of Math Destruction" if you want details on Big Data and insurance.
