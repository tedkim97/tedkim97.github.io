...
subtitle: A talk-through of an incomplete authentication system
published: false
enable_latex: false
permalink: /gaitkeep
frontpage: true
...
This post is about "GaitKeep", a group project that our team completed for a mobile computing class. GaitKeep was a multi-modal approach to biometric authentication that verified users through a combination of accelerometers (from a mobile device) and video-camera to increase security and consistency of gait authentication. GaitKeep was only 75-85% finished, but I thought it's still important to document our work and decisions we made.  
...
{% assign caption_feat = "Feature Concatenation Architecture. The ? indicate that we haven't figured out how a clean way of combining the data" %}
...
**Spoiler Alert**: Because of the failure in the accelerometer classification, that only the ensemble classifier was carried out :(. Note that the difference in shape of the accelerometer traces and video made feature concatenation very complicated. In addition, pooled classification will result in incredibly confusing network architectures. 
